...
function exportData() {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `vocab-enhancer-backup-${new Date().toISOString().split('T')[0]}.json`;
    a.click();
    URL.revokeObjectURL(url);
    ...
    reader.onload = async (e) => {
        try {
            ...
            if (data.words && Array.isArray(data.words)) {
                const existingWords = new Set(STATE.words.map(w => w.word));
                let addedCount = 0;
                ...
                if (data.streak && data.streak.count > STATE.streak.count) {
                    STATE.streak = data.streak;
                }
                saveState();
                renderHistory();
                updateHistoryStats();
                showToast(`Imported ${addedCount} new words successfully!`, 'success');
